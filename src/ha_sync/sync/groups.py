...
class GroupSyncer(BaseSyncer):
    ...
    def _expand_entity_id(self, entity_id: str, subtype: str) -> str:
        ...
        For group helpers, the entity domain matches the subtype (step_id).
        """
        if "." in entity_id:
            return entity_id
    ...
    def _validate_entity_id(self, entity_id: str, subtype: str) -> str | None:
        ...
        if actual_domain != subtype:
            return (
                f"Invalid entity_id domain '{actual_domain}' for group {subtype}. "
                f"Expected '{subtype}'."
            )
        return None
    ...
    async def get_remote_entities(self) -> dict[str, dict[str, Any]]:
        ...
        groups = await self.client.get_group_helpers()
        for helper in groups:
    ...
    def get_local_entities(self) -> dict[str, dict[str, Any]]:
        """Get all local group helper files."""
        result: dict[str, dict[str, Any]] = {}
        ...
        for full_id, data in remote.items():
            ...
            if model_class:
                try:
                    ...
                    # Fall back to raw config if validation fails
                    ordered = config
            else:
                ordered = config
